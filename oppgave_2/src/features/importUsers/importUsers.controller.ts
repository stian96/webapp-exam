...
import { writePerformersFromImport } from "./importUsers.service"
...
const importAll = async (): Promise<ApiResponse<string>> => {
    try {
        ...
        try {
            //TODO change dummyData to the real thing.

            const response: Result<string> = await writePerformersFromImport(dummyData);
            ...
            return {
                status: 500,
                message: "Unexpected error while writing to the database."
            };
        } catch (error: unknown) {
